...
For every genuinely new role, create a factual Markdown draft under `src/job-drafts` with `draft: true` and `review_status: needs-review`. Do not modify `src/jobs`, invent missing details, or publish jobs.
